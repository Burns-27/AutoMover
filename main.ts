...
import frontmatterUtil from "Utils/FrontmatterUtil";
...
export default class AutoMoverPlugin extends obsidian.Plugin {
	...
	async goThroughAllFiles(options: { silentIfUnchanged?: boolean } = {}) {
		...
		const results = await Promise.all(
			candidates.map((file) => this.matchAndMoveFile(file)),
		);
		...
		if (duplicate > 0)
			lines.push(
				`${duplicate} ${duplicate === 1 ? "file" : "files"} not moved - duplicate name.`,
			);
		if (failed > 0)
			lines.push(
				`${failed} ${failed === 1 ? "file" : "files"} failed to move.`,
			);

		loggerUtil.infoNotice(
			lines.length === 0
	...
	/**
	 * Matches the file to the first applicable rule (frontmatter, then project, then name, then tag)
	 * and moves it.
	 *
	...
	async matchAndMoveFile(file: obsidian.TFile): Promise<MoveOutcome | null> {
		...
		const frontmatterResult = await this.matchAndMoveByFrontmatter(file);
		if (frontmatterResult !== null) return frontmatterResult;
	...
	/**
	 * Matches File based on user defined properties, matching the top, middle, and 
   * end property in order. If there is no matching property/rule, it will default 
   * to the parent level. Each child level will append its folder to the parent's to
   * get the full path. If file has a matching rule, it is moved. 
	 * @param file
	 */
	async matchAndMoveByFrontmatter(
		file: obsidian.TFile,
  ): Promise<MoveOutcome | null> {
    const cache = this.app.metadataCache.getFileCache(file);
		if (cache == null) return null;
    ...
    const path = frontmatterUtil.matchFrontmatterRule(file, fm, this.settings.frontmatterRules, this.settings.frontmatterProperties);
    if (!path) return null;
    return movingUtil.moveFile(file, path)
  }
	/**
